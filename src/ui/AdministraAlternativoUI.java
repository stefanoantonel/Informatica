...
public class AdministraAlternativoUI extends JFrame {
	...
	public AdministraAlternativoUI(String id,String art, String cant, String um, ArrayList<String> altDesc, String[][] conv,ArrayList<String> articulosAlt) {
		  original=Integer.parseInt(id);
...
//					a.getNodoByDescripcion(art);
					...
					//Nodo viejo=new Nodo();
					//viejo=a.getNodoById(original);
					
					//CONTROLARR !!
					int nuevoId=a.getNodoByDescripcion(nuevo).getArt().getValor();
					ArrayList<Nodo> bomPH = new ArrayList<>();
					//devuelve padre e hijo para ese id 
					bomPH= a.getNodoByIdBom(original);
					
					aa.cambiarDefault((int) bomPH.get(0).getArt().getValor(), (int)bomPH.get(1).getArt().getValor(), nuevoId);
